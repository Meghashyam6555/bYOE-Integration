...
df = pd.read_csv("ssp_data.csv")
df2 = pd.read_csv("br.csv",";")
...
X = df_ml.drop(['Jobs get'], axis = 1).values
Y = df_ml['Jobs get'].values
...
app.layout = html.Div([html.H1("Data Analysis", style={"textAlign": "center"}), dcc.Markdown('''
Welcome to my Plotly (Dash) Data Science interactive dashboard.''')  ,
    dcc.Tabs(id="tabs", children=[
        dcc.Tab(label='Prices', children=[
html.Div([html.H1("Static Dashboard", style={'textAlign': 'center'}),
#dash_table.DataTable(
#    id='table',
#    columns=[{"name": i, "id": i} for i in df.columns],
#    data=df.iloc[0:5,:].to_dict("rows"),
#),
    html.H1("Broadcaster Prices High vs Lows", style={'textAlign': 'center', 'padding-top': 5}),
    dcc.Dropdown(id='my-dropdown',options=[{'label': 'T_BC', 'value': 'TSLA'},{'label': 'A_BC', 'value': 'AAPL'},{'label': 'F_BC', 'value': 'FB'},{'label': 'M_BC', 'value': 'MSFT'}],
    ...
    html.H1("Broadcaster Jobs Done over Time", style={'textAlign': 'center', 'padding-top': 5}),
    dcc.Dropdown(id='my-dropdown2',options=[{'label': 'T_BC', 'value': 'TSLA'},{'label': 'A_BC', 'value': 'AAPL'},{'label': 'F_BC', 'value': 'FB'},{'label': 'M_BC', 'value': 'MSFT'}],
        multi=True,value=['FB'],style={"display": "block", "margin-left": "auto", "margin-right": "auto", "width": "80%"}),
    dcc.Graph(id='volume'),
    html.H1("Scatter Analysis", style={'textAlign': 'center', 'padding-top': -10}),
    dcc.Dropdown(id='my-dropdown3',
                 options=[{'label': 'T_BC', 'value': 'TSLA'},{'label': 'A_BC', 'value': 'AAPL'},{'label': 'F_BC', 'value': 'FB'},{'label': 'M_BC', 'value': 'MSFT'}],
                 value= 'FB',
...
html.Div([html.H1("Broadcaster Metrics Distributions", style={"textAlign": "center"}),
...
dash_table.DataTable(
    ...
    data= df.describe().reset_index().to_dict("rows"),
            ...
            html.H1("Prime vs Non Prime by Category", style={'textAlign': "center", 'padding-top': 5}),
     html.Div([
         dcc.RadioItems(id="select-survival", value=str(1), labelStyle={'display': 'inline-block', 'padding': 10},
                        options=[{'label': "Price average", 'value': str(1)}, {'label': "Non Prime", 'value': str(0)}], )],
         style={'textAlign': "center", }),
...
html.Div([html.H1("Machine Learning", style={"textAlign": "center"}), html.H2("ARIMA Time Series Prediction", style={"textAlign": "left"}),
    dcc.Dropdown(id='my-dropdowntest',options=[{'label': 'T_BC', 'value': 'TSLA'},{'label': 'A_BC', 'value': 'AAPL'},{'label': 'F_BC', 'value': 'FB'},{'label': 'M_BC', 'value': 'MSFT'}],
...
html.H2("Performance Metrics Regression Prediction", style={"textAlign": "left"}), html.P("Training accuracy: "),
    str(train_acc), html.P("Test accuracy:"), str(test_acc),
...
def update_graph(selected_dropdown):
    dropdown = {"TSLA": "T_BC","AAPL": "A_BC","FB": "F_BC","MSFT": "M_BC",}
    trace1 = []
    trace2 = []
...
def update_graph(selected_dropdown_value):
    ...
    figure = {'data': data,
        'layout': go.Layout(colorway=["#5E0DAC", '#FF4F00', '#375CB1', '#FF7400', '#FFF400', '#FF0056'],
            height=600,title=f"Jobs Done for {', '.join(str(dropdown[i]) for i in selected_dropdown_value)} Over Time",
            xaxis={"title":"Date",
                   'rangeselector': {'buttons': list([{'count': 1, 'label': '1M', 'step': 'month', 'stepmode': 'backward'},
                                                      {'count': 6, 'label': '6M', 'step': 'month', 'stepmode': 'backward'},
                                                      {'step': 'all'}])},
...
def update_graph(stock, stock2, radioval):
    dropdown = {"TSLA": "T_BC","AAPL": "A_BC","FB": "F_BC","MSFT": "M_BC", }
    radio = {"High": "High Performance", "Low": "Low Performance", "Volume": "Market Volume", }
    trace1 = []
...
def update_graph(selected):
    dff = df2[df2["Price average"] == int(selected)]
